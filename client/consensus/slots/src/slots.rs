...
use sp_inherents::{InherentData, InherentDataProvider, CreateInherenData};
...
impl<Block, C, IDP> Stream for Slots<C, IDP>
where
	Block: BlockT,
	C: SelectChain<Block>,
	IDP: InherentDataProvider<()>,
	IDP::InherentDataProvider: crate::InherentDataProviderExt,
	IDP::Error: Into<Error>,
{
	...
	fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Option<Self::Item>> {
		loop {
			...
			let inherent_data_provider = match self.inherent_data_provider
				.create_inherent_data_provider(&BlockId::Hash(chain_head.hash()))
			{
				Ok(id) => id,
				Err(err) => return Poll::Ready(Some(Err(err.into()))),
			};

			let timestamp = inherent_data_provider.timestamp();
			let slot_num = inherent_data_provider.slot();
			let inheren_data = inherent_data_provider.create_inherent_data();
